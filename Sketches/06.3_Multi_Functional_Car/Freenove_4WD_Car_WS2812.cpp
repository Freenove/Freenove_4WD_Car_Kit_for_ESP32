...
void WS2812_Set_Color_1(int number, unsigned char color_1,unsigned char color_2,unsigned char color_3)    //Set the display color1 for WS2812
{
  ...
  {
    ws2812_strip_color_1[number - 1][0] = constrain(color_1, 0, 255);
    ws2812_strip_color_1[number - 1][1] = constrain(color_2, 0, 255);
    ws2812_strip_color_1[number - 1][2] = constrain(color_3, 0, 255);
  }
}
...
void WS2812_Setup(void)
{
  ...
  ws2812_close();
  WS2812_Set_Color_1(0, 0,0,100);
  WS2812_Set_Color_2(0, 0,0,0);
...
{
  ws2812_task_mode = constrain(mode, 0, 4);
}
...
void WS2812_Show(int mode)
{
  switch (mode)
  {
    case 0://Close the WS2812
      ws2812_close();
      break;
    case 1://After turning on this function, the user can adjust the color and brightness of the RGB lamp for collective display
      ws2812_following();
      break;

    case 2://When this function is enabled, the user can adjust the color and brightness of the RGB lamp to display one by one
      ws2812_blink();
      break;

    case 3://When this function is enabled, the user can adjust the color and brightness of the RGB lamp to blink collectively
      ws2812_breathe();
      break;

    case 4://After turning on this function, the user can adjust the color of the RGB lamp and breathe together
      ws2812_rainbow();
      break;
    default:
      break;
